...
### Relay + withdrawal bridge flags (mainnet)

| Variable | Description |
|----------|-------------|
| `MAINNET_BRIDGE_ENABLED` | Enables mainnet Unit/deBridge relay for withdrawals |
...
| `NEXT_PUBLIC_HCLAW_*_MAINNET` / `NEXT_PUBLIC_HCLAW_*_TESTNET` | Optional per-network overrides for token/lock/policy/rewards/agentic |
| `NEXT_PUBLIC_MONAD_MAINNET_HCLAW_*` / `NEXT_PUBLIC_MONAD_TESTNET_HCLAW_*` | Alternate per-network naming supported by runtime resolver |
| `MONAD_MAINNET_HCLAW_*` / `MONAD_TESTNET_HCLAW_*` | Server-only per-network overrides (API/runtime) |
...
- `NEXT_PUBLIC_HCLAW_*_MAINNET`
- `NEXT_PUBLIC_HCLAW_*_TESTNET`
