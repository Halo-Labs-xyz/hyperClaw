---
name: ms-5
description: Execute MS-5 (WS-5) with multi agent discipline using strict branch policy, dependency ordering, file ownership isolation, validation, and deterministic merge sequencing.
---
...
```text
Execute MS-5 (WS-5) with multi agent discipline. MAKE NO MISTAKES.
...
Session 1: branch feat/ms5-intent-contracts. MAKE NO MISTAKES.
Scope: WS-5.1 typed runtime artifacts and wiring.
Owned files only:
...
Session 2: branch feat/ms5-hyperliquid-tool. MAKE NO MISTAKES.
Dependency: start after Session 1 commit is available.
Scope: WS-5.2 Hyperliquid tool implementation + registry integration.
Owned files only:
...
Session 3: branch feat/ms5-hyperclaw-bridge-routes. MAKE NO MISTAKES.
Dependency: start after Session 1 and Session 2 commits are available.
Scope: WS-5.3 bridge routes for intents/execute/verify/runs.
Owned files only:
...
Session 4: branch docs/ms5-intent-execution-pipeline. MAKE NO MISTAKES.
...
Merge order:
1. feat/ms5-intent-contracts
2. feat/ms5-hyperliquid-tool
3. feat/ms5-hyperclaw-bridge-routes
4. docs/ms5-intent-execution-pipeline
```
